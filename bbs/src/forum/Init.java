...
public class Init {
	...
	public static void main(String[] args) {
		...
		String userPassword = "123456";// 管理员密码
		...
		try {
			...
			String path = user_dir + File.separator + "webapp" + File.separator + "WEB-INF" + File.separator + "data"
			...
			String sql = "INSERT INTO `sysusers` (`userId`,`enabled`,`fullName`,`issys`,`securityDigest`,`userAccount`,`userDesc`,`userDuty`,`userPassword`) VALUES ('"
					+ UUIDUtil.getUUID32() + "',b'1','" + userAccount.trim() + "',b'1','" + UUIDUtil.getUUID32() + "','"
					+ userAccount.trim() + "',NULL,'管理员','" + newPassword + "')";

			stmt = conn.createStatement();
			stmt.executeUpdate("delete from sysusers where 1=1");
	...
	private static Connection getConnection() throws Exception {
		...
		return DriverManager.getConnection(install.getDatabaseURL().trim(), install.getDatabaseUser().trim(),
				install.getDatabasePassword().trim());
	...
	private static Install getDatabaseParameter() {
		...
		try {
			String user_dir = System.getProperty("user.dir");
			String path = user_dir + File.separator + "src" + File.separator + "main" + File.separator + "resources"
					+ File.separator + "druid.properties";
