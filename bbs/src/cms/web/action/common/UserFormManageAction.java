	...
	public String register(ModelMap model,User formbean,
		...
		if(error.size() == 0){
			...
			if(error.size() == 0){
				...
				oAuthManage.addAccessToken(accessToken, new AccessUser(user.getId(),user.getUserName(),user.getSecurityDigest(),false));
				oAuthManage.addRefreshToken(refreshToken, new RefreshUser(accessToken,user.getId(),user.getUserName(),user.getSecurityDigest(),false));
		...
		if(error.size() == 0){
			...
			if(user != null){
				...
				if(user.getState() >1 ){
					...
					oAuthManage.addAccessToken(accessToken, new AccessUser(user.getId(),user.getUserName(),user.getSecurityDigest(),rememberMe));
					oAuthManage.addRefreshToken(refreshToken, new RefreshUser(accessToken,user.getId(),user.getUserName(),user.getSecurityDigest(),rememberMe));
					...
					AccessUserThreadLocal.set(new AccessUser(user.getId(),user.getUserName(),user.getSecurityDigest(),rememberMe));
